...
.PHONY : clean clean-all run zip req6 all sensor_gateway
...
#//////////TESTING AREA//////////
req6:
	gcc -Wall -Werror -lpthread -g main.c connmgr.h connmgr.c datamgr.c datamgr.h sensor_db.h sensor_db.c config.h sbuffer.c sbuffer.h lib/dplist.c lib/dplist.h lib/tcpsock.c lib/tcpsock.h -D SET_MIN_TEMP=13.8 -D SET_MAX_TEMP=17 -D TIMEOUT=5 -D MAX_CONN=3 -o req6
	./req6 5678
memcheck:
	valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose --log-file=valgrind.txt ./req6
	micro valgrind.txt
check:
	cppcheck --enable=all --suppress=missingIncludeSystem --error-exitcode=1 main.c
#//////////END OF TESTING AREA//////////
